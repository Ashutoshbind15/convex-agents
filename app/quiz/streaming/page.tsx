...
export default function StreamingPage() {
    ...
    useEffect(() => {
        const healthCheck = async () => {
            const response = await fetch(`${process.env.NEXT_PUBLIC_CONVEX_HTTP_ENDPOINT}/health`, {
                method: "POST",
                body: JSON.stringify({
                    msg: "Hello"
                })
...
function QuizConversation({ threadId }: { threadId: string }) {
    ...
    const { object, submit, clear, isLoading } = useObject({
        api: `${process.env.NEXT_PUBLIC_CONVEX_HTTP_ENDPOINT}/quiz/streamchat`,
        schema: z.object({
